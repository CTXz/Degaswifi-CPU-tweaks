...
#define IRQ_PXA1L88_AP_TIMER1	(IRQ_PXA1L88_START + 13)
#define IRQ_PXA1L88_AP_TIMER2_3	(IRQ_PXA1L88_START + 14)
#define IRQ_PXA1L88_AP2_TIMER1	(IRQ_PXA1L88_START + 29)
#define IRQ_PXA1L88_AP2_TIMER2_3	(IRQ_PXA1L88_START + 30)
...
static void pxa1L88_set_wake(int irq, unsigned int on)
{
	...
	case IRQ_PXA1L88_KEYPAD:
		...
		break;
	/* wakeup line 4 */
	case IRQ_PXA1L88_AP_TIMER1:
		awucrm = PMUM_WAKEUP4 | PMUM_AP1_TIMER_1;
		apcr |= PMUM_SLPWP4;
		break;
	case IRQ_PXA1L88_AP_TIMER2_3:
	...
	if (on) {
		...
		if (apcr) {
			apcr = ~apcr & __raw_readl(get_apmu_base_va() + APCR);
			__raw_writel(apcr, get_apmu_base_va() + APCR);
		}
	} else {
		if (awucrm) {
		...
		if (apcr) {
			apcr |= __raw_readl(get_apmu_base_va() + APCR);
			__raw_writel(apcr, get_apmu_base_va() + APCR);
		}
	}
}
