...
static int pxa2xx_spi_probe(struct platform_device *pdev)
{
	...
	if (dev->of_node) {
		...
		ssp = pxa_ssp_request_of(ssp_handle, "spi");
		if (ssp == NULL) {
			status = -ENODEV;
			goto out_error;
		}
...
static struct of_device_id pxa_spi_of_match[] = {
	{ .compatible = "marvell,pxa2xx-spi", },
	{}
};
MODULE_DEVICE_TABLE(of, pxa_spi_of_match);
...
static struct platform_driver driver = {
	.driver = {
		...
		.of_match_table = pxa_spi_of_match,
