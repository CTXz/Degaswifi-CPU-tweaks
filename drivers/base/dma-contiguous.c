...
static int cma_dt_reserve(phys_addr_t limit) {

	if (of_scan_flat_dt(cma_fdt_find_info, NULL)) {

		pr_debug("%s: reserving %ld MiB for global area\n", __func__,
		(unsigned long)cma_size_bytes / SZ_1M);
		dma_declare_contiguous(NULL, cma_size_bytes, 0, limit);
