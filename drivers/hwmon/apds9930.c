...
#define APDS9930_PS_DETECTION_THRESHOLD		600
#define APDS9930_PS_HSYTERESIS_THRESHOLD	500
...
static int luxcalculation(struct i2c_client *client, int ch0data, int ch1data)
{
	...
	if (data->als_reduce) {
		luxvalue =
			((IAC * APDS9930_GA * APDS9930_DF) / 100) * 4 /
			((apds9930_als_integration_tb[data->als_atime_index] / 100) *
			 apds9930_als_again_tb[data->als_again_index]);
	} else {
		luxvalue =
			((IAC * APDS9930_GA * APDS9930_DF) / 100) /
			((apds9930_als_integration_tb[data->als_atime_index] / 100)
			 * apds9930_als_again_tb[data->als_again_index]);
	}
...
static void apds9930_change_ps_threshold(struct i2c_client *client)
{
	...
	if ((data->ps_data > data->pilt) && (data->ps_data >= data->piht)) {
		...
		i2c_smbus_write_word_data(client, CMD_WORD | APDS9930_PILTL_REG, data->ps_hysteresis_threshold);
	...
	} else if ((data->pilt == 1023) && (data->piht == 0)) {	/* force interrupt */
		/* special case */
		/* near-to-far detected */
		data->ps_detection = 0;
...
static void apds9930_change_als_threshold(struct i2c_client *client)
{
	...
	pr_debug("lux=%d ch0data=%d ch1data=%d again=%d als_reduce=%d\n", luxvalue, ch0data, ch1data,
	...
	if (lux_is_valid) {
		input_report_abs(data->input_dev_als, ABS_LIGHT, luxvalue); /* report the lux level */
	...
	if (data->als_data >= (apds9930_als_res_tb[data->als_atime_index] * 90) / 100) {
		...
		if (data->als_again_index != APDS9930_ALS_GAIN_1X) {
			...
			err = i2c_smbus_write_byte_data(client, CMD_BYTE | APDS9930_CONFIG_REG, APDS9930_ALS_REDUCE);
			if (err >= 0)
				data->als_reduce = 1;
		}
		...
		if (data->als_reduce) {
			err = i2c_smbus_write_byte_data(client, CMD_BYTE | APDS9930_CONFIG_REG, 0);
			if (err >= 0)
				data->als_reduce = 0;
	...
	if (change_again) {
		control_data = i2c_smbus_read_byte_data(client, CMD_BYTE | APDS9930_CONTROL_REG);
		control_data = control_data & 0xFC;
		control_data = control_data | apds9930_als_again_bit_tb[data->als_again_index];
		i2c_smbus_write_byte_data(client, CMD_BYTE | APDS9930_CONTROL_REG, control_data);
	}

	i2c_smbus_write_word_data(client, CMD_WORD | APDS9930_AILTL_REG, data->als_threshold_l);
	i2c_smbus_write_word_data(client, CMD_WORD | APDS9930_AIHTL_REG, data->als_threshold_h);
...
static void apds9930_work_handler(struct work_struct *work)
{
	...
	i2c_smbus_write_byte_data(client, CMD_BYTE | APDS9930_ENABLE_REG, 1);	/* disable 993x's ADC first */
...
static DEVICE_ATTR(proximity_enable, S_IRUGO | S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(light_enable, S_IRUGO | S_IWUSR | S_IWGRP,
...
static int apds9930_init_client(struct i2c_client *client)
{
	...
	err = apds9930_set_wtime(client, 0xF6);	/* 27.2ms Wait time */
	if (err < 0)
		return err;
	...
	err = apds9930_set_control(client, APDS9930_PDRVIE_100MA | APDS9930_PRX_IR_DIOD
			| APDS9930_PGAIN_4X | apds9930_als_again_bit_tb[data->als_again_index]);
	if (err < 0)
		return err;
	...
	err = apds9930_set_pers(client, APDS9930_PPERS_2 | APDS9930_APERS_2);	/* 2 consecutive Interrupt persistence */
	if (err < 0)
		return err;
				   ...
				   const struct i2c_device_id *id)
{
	...
	data->input_dev_als->name = "avago_light_sensor";
	...
	data->input_dev_ps->name = "avago_prox_sensor";
