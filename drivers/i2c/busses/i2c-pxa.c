...
static int mmp_pinctrl_set_i2c(struct i2c_adapter *adap, char *name)
{
	struct pinctrl *p;
	...
	if (IS_ERR(p)) {
		pr_warn("AP: fail to get i2c adapter pinctrl!\n");
		return PTR_ERR(p);
	}

	s = pinctrl_lookup_state(p, name);
	if (IS_ERR(s)) {
		pr_warn("AP: fail to lookup i2c adpater default pin state!\n");
		return PTR_ERR(s);
	}

	ret = pinctrl_select_state(p, s);
	if (ret < 0) {
		pr_warn("AP: fail to set i2c adpater pin state!\n");
		return ret;
	}
...
void mmp_hwlock_lock(struct i2c_adapter *adap)
{
	...
	if (i2c->apdcp)
		mmp_pinctrl_set_i2c(adap, PINCTRL_STATE_DEFAULT);
...
void mmp_hwlock_unlock(struct i2c_adapter *adap)
{
	...
	if (i2c->apdcp)
		mmp_pinctrl_set_i2c(adap, PINCTRL_STATE_IDLE);
