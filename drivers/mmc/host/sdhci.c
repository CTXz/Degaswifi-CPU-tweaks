...
static struct mmc_request *gmrq;
...
static void sdhci_init(struct sdhci_host *host, int soft)
{
	...
	irq_set = SDHCI_INT_BUS_POWER | SDHCI_INT_DATA_END_BIT |
...
void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	if ((host->quirks2 & SDHCI_QUIRK2_SDIO_SW_CLK_GATE) &&
			...
			(host->mmc->caps2 & MMC_CAP2_BUS_AUTO_CLK_GATE) && cmd->opcode == 53)
		host->ops->clk_gate_auto(host, 1);
}
...
static void sdhci_tasklet_finish(unsigned long param)
{
	...
	if (host->mmc->card && host->mmc->card->type == MMC_TYPE_MMC
		&&  gmrq != mrq)
		printk("[MMC_TEST] host->mrq 0x%08x   gmrq 0x%08x \n", mrq, gmrq);
