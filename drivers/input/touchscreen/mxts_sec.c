...
static void module_off_master(void *device_data)
{
	...
	mutex_lock(&data->input_dev->mutex);
	snprintf(buff, sizeof(buff), "%s", (mxt_stop(data)) ? "NG" : "OK");
	mutex_unlock(&data->input_dev->mutex);

	set_default_result(fdata);
	set_cmd_result(fdata, buff, strnlen(buff, sizeof(buff)));

	fdata->cmd_state = strncmp(buff, "OK", 2) ? CMD_STATUS_FAIL : CMD_STATUS_OK;
...
static void module_on_master(void *device_data)
{
	...
	mutex_lock(&data->input_dev->mutex);
	snprintf(buff, sizeof(buff), "%s", (mxt_start(data)) ? "NG" : "OK");
	mutex_unlock(&data->input_dev->mutex);
...
static void run_delta_read(void *device_data)
{
	...
	set_default_result(fdata);
	fdata->cmd_state = mxt_read_all_diagnostic_data(data, MXT_DIAG_DELTA_MODE) ? CMD_STATUS_FAIL : CMD_STATUS_OK;
}
...
static int mxt_read_message_on_fcal(struct mxt_data *data, struct mxt_message *message)
{
	struct device *dev = &data->client->dev;
	struct i2c_client *client = data->client;
				  ...
				  *devattr, const char *buf, size_t count)
{
	...
	(cur = strchr(buf, (int)delim)) ? memcpy(buff, buf, cur - buf) : memcpy(buff, buf, len);
				 ...
				 size_t size)
{
	...
	/* control led */
	(input == 1) ? data->pdata->led_power_on() : data->pdata->led_power_off();
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &i) == 1 && i < 2) {
		...
		(data->atmeldbg.block_access) ? disable_irq(client->irq) : enable_irq(client->irq);
