	...
	if (do_action) {
		switch (action_id) {
			...
			case MXT_PATCH_ACTION_CAL:
				__mxt_patch_debug(data, "|-- ACTION CALIBRATE\n");
			...
			struct touch_pos* tpos, u8 tid, u16 x, u16 y)
{
	...
	return ((distance < tpos->distance) && (abs(diffx) < tpos->maxdiff) && (abs(diffy) < tpos->maxdiff)) ?
								true : false;
...
{
	if (do_action) {
		__mxt_patch_debug(data, "CHARG:%d FCNT:%d AREA:%d AMP:%d"
			" SUM:%d TCH:%d ATCH:%d\n",
		...
		u8* ppatch, u16* psrc_item, u16* check_cnt, bool do_action)
{
	...
	for(i = 0; i <ptline->cfg_cnt; i++) { /* Test Line Action config */
		...
		if (!do_action) {
			__mxt_patch_debug(data, "|-- ACTION_CFG: OBJECT_TYPE:%d"
				" OFFSET:%d VAL:%d\n",
				pacfg->obj_type, pacfg->offset, pacfg->val);
		}
		...
		u8* ppatch, u16* ptline_addr, u8* ptline_cnt, bool do_action)
{
	...
	for(i = 0; i < psdef->cfg_cnt; i++) { /* Stage Config Parsing */
		...
		if (!do_action) {
			__mxt_patch_debug(data,
				"|- STAGE_CFG: OBJECT_TYPE:%d OFFSET:%d VAL:%d\n",
				pscfg->obj_type, pscfg->offset, pscfg->val);
		}
			...
			u8 object, u8 index, u8* pmsg)
{
	...
	return ((data->reportids[reportid].type == object) && (data->reportids[reportid].index == index)) ? 0 : 1;
}
...
static int mxt_patch_start_stage(struct mxt_data *data)
{
	if (data->patch.patch) {
		...
		__mxt_patch_debug(data, "PATCH: START STAGE\n");
#if MXT_PATCH_LOCK_CHECK
