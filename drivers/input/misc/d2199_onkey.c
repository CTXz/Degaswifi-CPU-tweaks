...
/* AT + KEYSHORT cmd */
static ssize_t keys_read(struct device *dev, struct device_attribute *attr,
								char *buf)
{
	return (powerkey_pressed != 0)? sprintf(buf, "%s\n", "PRESS") : sprintf(buf, "%s\n", "RELEASE");
}
static DEVICE_ATTR(sec_power_key_pressed, S_IRUGO, keys_read, NULL);

static int d2199_onkey_probe(struct platform_device *pdev)
{
	...
	dlg_onkey->input->id.bustype = BUS_I2C;
	...
	d2199_register_irq(d2199, D2199_IRQ_ENONKEY_HI,
				d2199_onkey_event_hi_handler,
				0, DRIVER_NAME, d2199);
	d2199_register_irq(d2199, D2199_IRQ_ENONKEY_LO,
				d2199_onkey_event_lo_handler,
				0, DRIVER_NAME, d2199);
	dev_info(d2199->dev, "Onkey Driver registered\n");

#if 0
	sec_powerkey = device_create(sec_class, NULL, 0, "%s",
						"sec_power_key");
	if (device_create_file(sec_powerkey,
				&dev_attr_sec_power_key_pressed) < 0)
		pr_err("d2199_onkey: Failed to create device file(%s)!\n",
				dev_attr_sec_power_key_pressed.attr.name);
#endif
	return 0;
}

static int d2199_onkey_remove(struct platform_device *pdev)
{
	struct d2199 *d2199 = platform_get_drvdata(pdev);
	struct d2199_onkey *dlg_onkey = &d2199->onkey;
