...
//#define CANNOT_WAKEUP_BY_LONG_DEBOUNCETIME
#ifdef CANNOT_WAKEUP_BY_LONG_DEBOUNCETIME
...
#ifdef CANNOT_WAKEUP_BY_LONG_DEBOUNCETIME
	if (button->gpio == wakeup_gpio)
	{
		//pr_info("wake up by GPIO %d. Set it to press : state : %d\n", wakeup_gpio, state);
		state =1;
		wakeup_gpio = -1;
	}
#endif	
...
//	pr_info("gpio_key : irq happned for gpio %d\n", bdata->button->gpio);
	if (bdata->button->wakeup)
				...
				struct device_attribute *attr, char *buf)
{
	...
	for ( i = 0; i < ddata->pdata->nbuttons; i++) {
		state |= ddata->data[i].key_state;
		pr_info("junik button %d\n", ddata->data[i].key_state);
	}
...
#ifdef CANNOT_WAKEUP_BY_LONG_DEBOUNCETIME
void gpio_key_wakeup(int gpio, void *data) 
{ 
	//pr_info("gpio_key_wakeup : GPIO Key wakeup (%d).\n", gpio);
	wakeup_gpio = gpio;
...
static int gpio_keys_probe(struct platform_device *pdev)
{
 ...
 #ifndef CONFIG_KEYBOARD_PXA27x /* This is only for "not use PXA7x_keypad", for decreasing current consumption */
	u32 kpc;
...
#ifdef CANNOT_WAKEUP_BY_LONG_DEBOUNCETIME
		ret = request_mfp_edge_wakeup(pdata->edge_wakeup_gpio, gpio_key_wakeup, (void *)pdata, dev);
#else
		ret = request_mfp_edge_wakeup(pdata->edge_wakeup_gpio, NULL, (void *)pdata, dev);
#endif
		if (ret)
...
#ifdef CONFIG_MACH_PXA_SAMSUNG
	ddata->sec_key = device_create(sec_class, NULL, (dev_t)NULL,
									ddata, "sec_key");
	if (IS_ERR(ddata->sec_key)) {
		error = PTR_ERR(ddata->sec_key);
		dev_err(dev, "Failed to create device for sec key(%d)\n", error);
	}
