...
void d2199_irq_worker(struct work_struct *work)
{
	...
	for (i = 0; i < ARRAY_SIZE(d2199_irqs); i++) {
		...
		if (!read_done[data->reg]) {
			d2199_reg_read(d2199, D2199_EVENT_A_REG +
					 data->reg, &reg_val);
			sub_reg[data->reg] = reg_val;
			d2199_reg_read(d2199, D2199_IRQ_MASK_A_REG +
					 data->reg, &reg_val);
			sub_reg[data->reg] &= ~reg_val;
	...
	if (sub_reg[data->reg] & data->mask) {
		d2199_irq_call_handler(d2199, i);
			...
			d2199_set_bits(d2199, D2199_EVENT_A_REG +
			 data->reg, d2199_irqs[i].mask);
		}
	}
...
static irqreturn_t d2199_irq(int irq, void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(d2199_irqs); i++) {
		...
		if (sub_reg[pIrq->reg] & pIrq->mask) {
			...
			/* Now clear EVENT registers */
			d2199_set_bits(d2199, D2199_EVENT_A_REG +
			 pIrq->reg, d2199_irqs[i].mask);
			/*dev_info(d2199->dev, "\nIRQ Register[%d]
			 MASK[%d]\n",D2199_EVENT_A_REG + pIrq->reg,
			 d2199_irqs[i].mask);*/
		}
	}
	return IRQ_HANDLED;
...
int d2199_unmask_irq(struct d2199 *d2199, int irq)
{
	dev_info(d2199->dev, "\nIRQ[%d] Register[%d] MASK[%d]\n", irq,
	 D2199_IRQ_MASK_A_REG + d2199_irqs[irq].reg, d2199_irqs[irq].mask);
	return d2199_clear_bits(d2199, D2199_IRQ_MASK_A_REG +
				 d2199_irqs[irq].reg,
				 d2199_irqs[irq].mask);
}
		    ...
		    struct d2199_platform_data *pdata)
{
	...
	reg_data = 0xFFFFFF;
	d2199_block_write(d2199, D2199_EVENT_A_REG,
	 D2199_NUM_IRQ_EVT_REGS, (u8 *)&reg_data);
	reg_data = 0;
	d2199_block_write(d2199, D2199_EVENT_A_REG,
	 D2199_NUM_IRQ_EVT_REGS, (u8 *)&reg_data);
	...
	maskbit = 0xFFFFFF;
	d2199_block_write(d2199, D2199_IRQ_MASK_A_REG,
	 D2199_NUM_IRQ_MASK_REGS, (u8 *)&maskbit);

	mutex_init(&d2199->irq_mutex);
	irq_set_irq_type(irq, IRQF_TRIGGER_FALLING);	/* test */

	printk(KERN_INFO "[WS] irq[%d]\n", irq);

	if (irq) {
		ret = request_threaded_irq(irq, NULL, d2199_irq, IRQF_ONESHOT,
								"d2199", d2199);
	if (ret != 0) {
		dev_err(d2199->dev,
