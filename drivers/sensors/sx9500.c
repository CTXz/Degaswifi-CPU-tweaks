...
#define THRESHOLD_TA_H	19
#define THRESHOLD_TA_L	10
#elif defined(CONFIG_MACH_GOYA)
#define THRESHOLD_TA_H	17
#define THRESHOLD_TA_L	4
#else
#define THRESHOLD_TA_H	17
#define THRESHOLD_TA_L	4
#endif
...
static void sx9500_send_event(struct sx9500_p *data, int cnt, u8 state)
{
	if (state == ACTIVE) {
		data->state[cnt] = ACTIVE;
		sx9500_i2c_write(data, SX9500_CPS_CTRL6_REG, touchTh_ta);
		pr_info("[SX9500]: %s - %d button touched\n", __func__, cnt);
	} else {
		data->touchMode = NORMAL_TOUCH_MODE;
		data->state[cnt] = IDLE;
		sx9500_i2c_write(data, SX9500_CPS_CTRL6_REG, releaseTh_ta);
...
static void sx9500_touchCheckWithRefSensor(struct sx9500_p *data)
{
	...
	if (data->state[cnt] == IDLE) {
		if (capMain >= threshold)
			sx9500_send_event(data, cnt, ACTIVE);
		else
			sx9500_send_event(data, cnt, IDLE);
	} else {
		if (capMain < threshold)
			sx9500_send_event(data, cnt, IDLE);
		else
...
static int sx9500_do_calibrate(struct sx9500_p *data, bool do_calib)
{
	...
	if ((data->calData[2] >= LIMIT_PROXOFFSET)
		|| (data->calData[2] == 0)) {
		pr_err("[SX9500]: %s - offset warning(%d)\n", __func__,
			data->calData[2]);
//		goto cal_fail;
	}
...
//	data->touchTH = data->releaseTh = (u8)val;
	touchTh_ta = releaseTh_ta = (u8)val;
	return count;
}
...
#if !defined(CONFIG_MACH_GOYA) && !defined(CONFIG_MACH_DEGAS)
/* GOYA call the charger_status_cb before the initialize_variable */
	touchTh_ta = (u8)CONFIG_SENSORS_SX9500_NORMAL_TOUCH_THRESHOLD;
	releaseTh_ta = (u8)CONFIG_SENSORS_SX9500_NORMAL_TOUCH_THRESHOLD;
#endif
	data->touchMode = INIT_TOUCH_MODE;
	data->flagDataSkip = false;
	data->calSuccessed = false;
...
/* changing grip sensor threshold at TA/USB or no cable status - H/W request */
#if defined(CONFIG_MACH_GOYA) || defined(CONFIG_MACH_DEGAS)
static void select_value(int status)
{
	switch(status) {
	case POWER_SUPPLY_TYPE_MAINS:		/* TA */
	case POWER_SUPPLY_TYPE_MISC:		/* USB */
	case POWER_SUPPLY_TYPE_USB:		/* USB */
		pr_info("[SX9500]: charger status USB or TA\n");
		touchTh_ta = releaseTh_ta = THRESHOLD_TA_H;
		break;
	case POWER_SUPPLY_TYPE_UNKNOWN:		/* ignore(skip) */
	case POWER_SUPPLY_TYPE_BATTERY:		/* no cable */
		pr_info("[SX9500]: charger status no cable\n");
		touchTh_ta = releaseTh_ta = THRESHOLD_TA_L;
		break;
	}
}

void charger_status_cb(int status)
{
	pr_info("[SX9500]: charger status %d\n", status);
	select_value(status);
}
EXPORT_SYMBOL(charger_status_cb);
#endif
...
static int sx9500_parse_dt(struct sx9500_p *data, struct device *dev)
{
	...
	data->gpioNirq = of_get_named_gpio_flags(dNode,	"grip_Nirq", 0, &flags);
	if (data->gpioNirq < 0) {
		...
		const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq((unsigned int)data->irq, NULL,
		sx9500_interrupt_thread, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
		"sx9500_irq", data);
	if (ret < 0) {
