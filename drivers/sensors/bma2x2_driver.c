...
struct bma2x2_data {
	...
	struct pinctrl_state *sleep_pins_state;
...
static int bma2x2_power_setup(struct device *dev, bool onoff) {

	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *info = i2c_get_clientdata(client);
	struct bma2x2_platform_data *pdata = info->pdata;
	int min_uv, max_uv;
	int ret = 0;

	if (onoff) {
		if (pdata->supply_type == REGULATOR_SUPPLY_TYPE) {
			pdata->bma2x2_vdd_regulator = regulator_get(dev, "v_sensor_2v85");
			if (unlikely(ret)) {
				dev_err(dev, "%s: error getting regulator v_sensor_2v85 (%d)!!\n", __func__, ret);
				return ret;
			}

			min_uv = max_uv = pdata->vdd_regulator_volt;
			ret = regulator_set_voltage(pdata->bma2x2_vdd_regulator, min_uv, max_uv);
			if (unlikely(ret)) {
				dev_err(dev, "%s: failed to set bma2x2_vdd_regulator to %d, %d (%d)\n",
					__func__, min_uv, max_uv, ret);
				return ret;
			}

			dev_info(dev, "%s: set bma2x2_vdd_regulator to %d uV - %d uV (%d)\n",
					__func__, min_uv, max_uv, ret);
		}
	}
	else {
		if (pdata->supply_type == REGULATOR_SUPPLY_TYPE) {
			regulator_force_disable(pdata->bma2x2_vdd_regulator);
			regulator_put(pdata->bma2x2_vdd_regulator);
		}
	}
...
static int bma2x2_power_onoff(struct device *dev, bool onoff) {
	...
	if (pdata->supply_type == REGULATOR_SUPPLY_TYPE) {
		...
		if (pdata->bma2x2_vdd_regulator == NULL) {
			dev_info(dev, "%s: no regulator.\n",__func__);
			return -1;
		}

		ret = (onoff) ? regulator_enable(pdata->bma2x2_vdd_regulator) :
				regulator_disable(pdata->bma2x2_vdd_regulator);
		if (unlikely(ret)) {
			dev_info(dev, "%s: error %s regulator\n", __func__,
					(onoff) ? "enabling" : "disabling");
		}
		else {
			dev_info(dev, "%s: %s (%d)\n", __func__, (onoff) ? "on" : "off", ret);
		}
	}
...
static int bma2x2_probe_dt(struct device_node *np, struct device *dev, struct bma2x2_platform_data *pdata) {
	...
	match = of_match_device(bma2x2_dt_ids, dev);
	if (!match) {
		dev_err(dev, "Compatible mismatch!!\n");
		return -EIO;
	}

	ret = of_property_read_u32(np, "bma2x2,supply_type", &pdata->supply_type);
	if (unlikely(ret)) {
	...
	if (pdata->supply_type == REGULATOR_SUPPLY_TYPE) {
		...
		ret = of_property_read_u32(np, "bma2x2,vdd_regulator_volt", &pdata->vdd_regulator_volt);
		if (unlikely(ret)) {
	...
	pdata->gpio_int = of_get_named_gpio(np, "bma2x2,gpio_int", 0);
	if (unlikely(pdata->gpio_int < 0)) {
		dev_err(dev, "error getting property gpio_int from device node\n");
		ret = pdata->gpio_int;
		goto error;
	}

#ifdef CONFIG_BMA_USE_PLATFORM_DATA
	ret = of_property_read_u32(np, "bma2x2,accel_place", &pdata->place);
	if (unlikely(ret)) {
	...
	pdata->power_setup = bma2x2_power_setup;
	pdata->power_onoff = bma2x2_power_onoff;
		...
		const struct i2c_device_id *id)
{
	...
	if (unlikely(!pdata)) {
		...
		ret = bma2x2_probe_dt(np, &client->dev, pdata);
		if (unlikely(ret)) {
	...
	if (data->sensor_type != -1) {
		...
		pr_err("Bosch Sensortec Device detected!\n"
				"%s registered I2C driver!\n",
						sensor_name[data->sensor_type]);
	} else {
		pr_err("Bosch Sensortec Device not found I2C error %d\n",
								tempvalue);
		err = -ENODEV;
		goto kfree_data;
	}
	...
	ret = gpio_request(pdata->gpio_int, "bma2x2_int");
	if (unlikely(ret < 0)) {
		dev_err(&client->dev, "error requesting gpio for interrupt\n");
	...
	if (NULL != client->dev.platform_data) {
		if (NULL != data->bst_pd)
			memcpy(data->bst_pd, client->dev.platform_data,
					sizeof(*data->bst_pd));
	} else if (pdata->place >= 0 && pdata->place <= 7) {
		if (NULL != data->bst_pd)
			data->bst_pd->place = pdata->place;
	}
#endif
	...
	ret = data->pdata->power_setup(&client->dev, true);
	if (ret) {
		dev_err(&client->dev, "Failed to register input dev\n");
		goto error_power_setup;
	}

	ret = data->pdata->power_onoff(&client->dev, true);
	if (ret) {
		dev_err(&client->dev, "Failed to register input dev\n");
		goto error_power_onoff;
	}
