...
static ssize_t bma255_postion_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct bma255_p *data = dev_get_drvdata(dev);
...
static ssize_t bma255_postion_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int chip_pos;
	int error;
	struct mutex chip_pos_mutex;
	struct bma255_p *data = dev_get_drvdata(dev);

	mutex_init(&chip_pos_mutex);
	error = kstrtoul(buf, 10, &chip_pos);
	if (error)
		return error;
	mutex_lock(&chip_pos_mutex);
	data->chip_pos = chip_pos;
	mutex_unlock(&chip_pos_mutex);
	mutex_destroy(&chip_pos_mutex);
	return count;
}
...
static DEVICE_ATTR(postion, S_IRUGO | S_IWUSR | S_IWGRP,
	bma255_postion_show, bma255_postion_store);
...
static int bma255_parse_dt(struct bma255_p *data, struct device *dev)
{
	...
	data->acc_int1 = of_get_named_gpio(dNode, "acc_int", 0);
	if (data->acc_int1 < 0) {
		pr_err("[SENSOR]: %s - error data->acc_int1 = %d \n",
			__func__, data->acc_int1);
		return -ENODEV;
	}

	if (of_property_read_u32(dNode,
			"acc_chip_pos", &data->chip_pos) < 0)
