...
#ifdef CONFIG_BMM_USE_PLATFORM_DATA
	client_data->bst_pd = kzalloc(sizeof(*client_data->bst_pd), GFP_KERNEL);
...
#ifdef CONFIG_OF
	if (NULL != client_data->bst_pd) {
		err = of_property_read_u32(np, "bmm050,magnetic_place", &client_data->bst_pd->place);
		if (unlikely(err)) {
			PERR("error reading property magnetic_place from device node %d\n",client_data->bst_pd->place);
			return err;
		}
	}
#endif
