...
static int flag_whoami;

static struct mpu_platform_data motion_platform_data = {
	.int_config  = 0x10,
	.level_shifter = 0,
	.orientation = {	1,  0,  0,
				0,  1,  0,
...
static struct mpu_platform_data motion_mpu6050_platform_data = {
	.int_config  = 0x10,
	.level_shifter = 0,
	.orientation = {	-1,  0,  0,
				0,  -1,  0,
...
static int set_power_itg(struct inv_mpu_iio_s *st, bool power_on)
{
	...
	u8 data;
	int result;
	if (power_on && INV_MPU9250 == st->chip_type)
		if (regulator_enable(st->avdd)) {
			dev_err(&st->client->dev, "mpu sensor regulator enable failed\n");
			goto out;
		}
...
#ifdef CONFIG_OF
static int inv_mpu_probe_dt(struct i2c_client *client)
{
	...
	platform_data->irq_gpio =
		of_get_named_gpio(np, "invensense,irq-gpios", 0);
	...
	const struct i2c_device_id *id)
{
	...
	if (IS_ERR(avdd)) {
		dev_err(&client->dev, "invensense mpu sensor avdd power supply get failed\n");
		goto out;
	}
	regulator_set_voltage(avdd, 2800000, 2800000);
	if (regulator_enable(avdd)) {
		dev_err(&client->dev, "invensense mpu sensor regulator enable failed\n");
		goto out;
	}
	...
	if (client->dev.of_node) {
		...
		if (result)
			goto out_no_free;
		if (whoami != MPU6500_ID && whoami != MPU9250_ID && whoami != MPU6050_ID)
			goto out_no_free;
	...
	retval = inv_mpu_probe_dt(client);
	if (retval == -ENOMEM) {
		dev_err(&client->dev,
		"%s: Failed to alloc mem for mpu_platform_data\n", __func__);
		goto out_no_free;
	...
	if (gpio_is_valid(platform_data->irq_gpio)) {
		...
		result = gpio_request(platform_data->irq_gpio, "inv_mpu_irq_gpio");
		if (result) {
		...
		st->client->irq = gpio_to_irq(platform_data->irq_gpio);
	} else {
	...
	if (client->dev.of_node) {
		if (flag_whoami == 1)
			st->plat_data = motion_mpu6050_platform_data;
		else
			st->plat_data = motion_platform_data;
	} else
