...
int alarm_set_rtc_ring(struct timespec alarm_time, enum android_alarm_type alarm_type)
{
	...
	if (pwr_rtc_dev != NULL) {
		...
		{
			ktime = ktime_sub(timespec_to_ktime(alarm_time), ktime_get_boottime());
		}

		alarm_time = ktime_to_timespec(ktime);
		rtc_alarm_time = alarm_time.tv_sec;
		alarm_dbg(INT, "%s, alarm time: %lu\n",
				__func__, rtc_alarm_time);
		rtc_read_time(pwr_rtc_dev, &tm);
		rtc_tm_to_time(&tm, &now);
		rtc_alarm_time += now - 120; //Power up Phone 120 seconds in advance.
...
void alarm_set_bootalarm(char *alarm_data)
{
	...
	if (alm.enabled) {
		...
		pr_info("alarm_set_bootalarm alarm disable\n");
...
static long alarm_do_ioctl(struct file *file, unsigned int cmd,
		struct timespec *ts, unsigned long arg)
{
	int rv = 0;
	unsigned long flags;
	...
	case ANDROID_ALARM_SET(0):
		...
		if (alarm_type == ANDROID_ALARM_POWER_UP)
			alarm_set_rtc_ring(new_alarm_time, alarm_type);
#endif
		break;
		/* fall though */
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	rv = alarm_do_ioctl(file, cmd, &ts, arg);
	if (rv)
		return rv;
