...
typedef int (*rxCallbackFunc) (const unsigned char *packet, int len,
			       unsigned char cid);
typedef int (*terminateCallbackFunc) (void);
typedef int (*authenticateCallbackFunc) (unsigned char cid,
...
typedef int (*marvell_modem_ioctl_notify_callback) (void *tty, unsigned int cmd,
						    unsigned long arg);
