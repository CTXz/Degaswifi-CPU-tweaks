...
{
	pppControl->PppTimer.expires =
	    jiffies + msecs_to_jiffies(CHAP_CHALLENGE_RESPONSE_TIMEOUT_IN_MSEC);
				      ...
				      U_CHAR success_or_fail_code)
{
	...
	char *pMessage =
	    (char *)message[success_or_fail_code - CHAP_AUTHENTICATE_SUCCESS];
...
#else
	PPP_PRINTF(KERN_ERR"[PPP] %s: Variable challenge length" \
