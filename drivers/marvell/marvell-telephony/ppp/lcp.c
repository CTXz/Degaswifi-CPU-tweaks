		...
		if (PPP_IS_BIT_SET
			...
			case LCP_MRU:
				PPP_APPEND(buf, index,
				    pppControl->lcpXmitParams.Mru.Header.Type,
				    fcs);
				PPP_APPEND(buf, index,
				    pppControl->lcpXmitParams.Mru.Header.Length,
				    fcs);
				PPP_APPEND_SHORT(buf, index,
				    pppControl->lcpXmitParams.Mru.Mru, fcs);
			...
			case LCP_ACCM:
				PPP_APPEND(buf, index,
				    pppControl->lcpXmitParams.Accm.Header.Type,
				    fcs);
				PPP_APPEND(buf, index,
				...
				PPP_APPEND_LONG(buf, index,
				    pppControl->lcpXmitParams.Accm.Accm, fcs);
				PPP_LOG_1(pppControl->lcpXmitParams.Accm.Accm);
				len += LCP_ACCM_LEN;
				break;

			case LCP_AUTH:
				if (pppControl->lcpXmitParams.AuthProtocol.Proto
				    == PPP_PAP) {
					PPP_APPEND(buf, index,
			...
			case LCP_MAGIC:
				...
				PPP_APPEND(buf, index,
				    ...
				    fcs);
				magic =
				    pppControl->lcpRecvParams.MagicNumber.
				    Number;
				PPP_LOG_1(magic);
				magic ^= 0x7EA5A57E;
				PPP_LOG_1(magic);
