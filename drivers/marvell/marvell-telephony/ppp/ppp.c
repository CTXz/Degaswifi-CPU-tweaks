...
static void pppStartTimer(PppControlS *pppControl, U_INT timeout,
...
int pppRelayMessageFromComm(const U_CHAR *buf, U_INT count, U_CHAR cid)
{
	...
	if (PppControl.PppState == PPP_STATE_CONNECTED) {
		...
		if (messageFromComm != NULL) {
			...
			/* One less available slot in queue */
			PppControl.QueuedDataInPacketsCount--;
...
static void pppProcessQMsg(QueueMessageS *qMsg)
{
	...
	case PPP_SEND_TERMINATE_REQ:
		pppStartTimer(&PppControl, PPP_TERMINATE_TIMEOUT,
	...
	case PPP_LCP_ECHO_REQ_TIMEOUT:
		...
		if (PppControl.lcpEchoReqTimeoutCount >=
			...
			PPP_PRINTF
			    ("[PPP] %s: %d echo requests failed on timeout, " \
	...
	do {
		...
		if (PppControl.IsRxFragmented) {
			...
			if (newMessage == NULL) {
				...
				PPP_PRINTF("[PPP] %s: Fragmented Flag Error" \
		...
		if (newMessage->Length > PppControl.lcpRecvParams.Mru.Mru + 3) {
			...
			PPP_PRINTF
			    ("[PPP] %s: Message too long, dropping..." \
			     "length=%d mru=%d\n",
			...
			if (PPP_GOODFCS == PppControl.IncomingFcs) {
				...
				PPP_LOG_3((U_INT)newMessage,
				    (U_INT)newMessage->Buf, newMessage->Length);
				PPP_TRACE_BUF("In-Decoded: ", newMessage->Buf,
				    newMessage->Length);
...
void PppUpdateIpParams(IpcpConnectionParamsS *inIpParams)
{
	...
	ipParams =
	    (IpcpConnectionParamsS *) PPP_MALLOC(sizeof(IpcpConnectionParamsS));
