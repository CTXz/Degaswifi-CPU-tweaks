...
extern int sec_fg_dt_init(struct device_node *np,
			struct device *dev,
...
#else
	fuelgauge->client = mfd_dev->client;
	if (IS_ENABLED(CONFIG_OF)) {
		if (!fuelgauge->pdata)
			fuelgauge->pdata =
				kzalloc(sizeof(sec_battery_platform_data_t),
								GFP_KERNEL);
		if (!fuelgauge->pdata)
			return -ENOMEM;
		node = of_find_node_by_name(NULL, "sec-fuelgauge");
		ret = sec_fg_dt_init(node, &pdev->dev, fuelgauge->pdata);
		if (ret)
			return ret;
	} else if (!fuelgauge->pdata) {
...
#elif defined(CONFIG_FUELGAUGE_88PM800)
...
static void sec_fuelgauge_shutdown(struct device *dev)
{
	...
	if (!sec_hal_fg_shutdown(fuelgauge))
		dev_err(&fuelgauge->client->dev,
			"%s: Failed to shutdown Fuelgauge\n", __func__);
}
