		     ...
		     unsigned long irq_flags)
{
	...
	int retval;
	unsigned long val, revision_register;
	...
	if (((unsigned long)ioaddr & (0x3e0 << SMC_IO_SHIFT)) != val) {
		pr_warn("%s: IOADDR %p doesn't match configuration (%lu).\n",
			CARDNAME, ioaddr, val);
	}
	...
	if (!version_string || (revision_register & 0xff00) != 0x3300) {
		...
		pr_warn("%s: IO %p: Unrecognized revision register 0x%04lu"
			", Contact author.\n", CARDNAME,
