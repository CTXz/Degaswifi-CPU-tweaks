 ...
 * phy list is used to pair phy/path when path/phy registered
 ...
 * phy driver do phy register/unregister
...
/*
 * mmp_register_phy - register phy to phy_list and connect to path
 * @p: phy to be registered
 *
 * this function provides interface for phy drivers to register phy
 * to phy_list and connect to path which matchs phy->plat_path_name.
 * no error returns when no matching path is found as path register after
 * phy register is permitted.
 */
void mmp_register_phy(struct mmp_phy *phy)
{
	struct mmp_path *path;
	...
	/* add */
	list_add_tail(&phy->node, &phy_list);

	/* try to register to path */
	list_for_each_entry(path, &path_list, node) {
		if (!strcmp(phy->plat_path_name, path->name)) {
			dev_info(phy->dev, "connect to path %s\n",
				path->name);
			path->phy = phy;
			break;
		}
...
EXPORT_SYMBOL_GPL(mmp_register_phy);

/*
 * mmp_unregister_phy - unregister phy from phy_list and disconnect
 * @p: phy to be unregistered
 *
 * this function provides interface for phy drivers to unregister phy
 * from phy_list and disconnect from path.
 */
void mmp_unregister_phy(struct mmp_phy *phy)
{
	struct mmp_path *path;
	...
	list_for_each_entry(path, &path_list, node) {
		if (path->phy && path->phy == phy) {
			dev_info(phy->dev, "disconnect from path %s\n",
				path->name);
			path->phy = NULL;
			break;
		}
...
EXPORT_SYMBOL_GPL(mmp_unregister_phy);
