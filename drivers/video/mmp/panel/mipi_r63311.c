...
static void r63311_panel_on(struct mmp_path *path)
{
	mmp_phy_dsi_tx_cmd_array(path->phy, r63311_display_on_cmds,
...
#else
static void r63311_panel_power(struct mmp_panel *panel, int on) {}
#endif

static void r63311_onoff(struct mmp_panel *panel, int status)
{
	...
	if (status) {
		...
		if (plat->plat_onoff)
			...
			r63311_panel_power(panel, 0, 1);
		r63311_panel_on(path);
	} else {
		/* power off */
		if (plat->plat_onoff)
			plat->plat_onoff(0);
...
static void r63311_reduced_onoff(struct mmp_panel *panel, int status)
{
	if (status) {
		/* power on */
#ifdef CONFIG_DDR_DEVFREQ
		if(panel->ddrfreq_qos != PM_QOS_DEFAULT_VALUE)
			pm_qos_update_request(&panel->ddrfreq_qos_req_min,
				panel->ddrfreq_qos);
#endif
		r63311_panel_power(panel, 1, 1);
	} else {
		/* power off */
		 r63311_panel_power(panel, 1, 0);
#ifdef CONFIG_DDR_DEVFREQ
		if(panel->ddrfreq_qos != PM_QOS_DEFAULT_VALUE)
			pm_qos_update_request(&panel->ddrfreq_qos_req_min,
				PM_QOS_DEFAULT_VALUE);
...
static struct mmp_panel panel_r63311 = {
	...
	.set_onoff = r63311_onoff,
	.reduced_onoff = r63311_reduced_onoff,
};
...
static int r63311_probe(struct platform_device *pdev)
{
	...
	ret = r63311_ls_bl_update_status(bl);
	if (ret < 0)
