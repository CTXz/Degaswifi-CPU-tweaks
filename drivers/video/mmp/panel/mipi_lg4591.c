...
static void lg4591_panel_on(struct mmp_path *path)
{
	mmp_phy_dsi_tx_cmd_array(path->phy, lg4591_display_on_cmds,
...
#else
static void lg4591_panel_power(struct mmp_panel *panel, int on) {}
#endif
...
static void lg4591_reduced_onoff(struct mmp_panel *panel, int status)
{
	if (status) {
		/* power on */
		lg4591_panel_power(panel, 1, 1);
	} else {
		/* power off */
		lg4591_panel_power(panel, 1, 0);
	}
}
...
static struct mmp_panel panel_lg4591 = {
	...
	.set_onoff = lg4591_onoff,
	.reduced_onoff = lg4591_reduced_onoff,
