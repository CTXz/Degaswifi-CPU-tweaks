...
static void otm1281_panel_on(struct mmp_path *path)
{
	mmp_phy_dsi_tx_cmd_array(path->phy, otm1281_display_on_cmds,
...
#else
static void otm1281_panel_power(struct mmp_panel *panel, int on) {}
#endif
static void otm1281_onoff(struct mmp_panel *panel, int status)
{
	...
	if (status) {
		...
		if (plat->plat_onoff)
			...
			otm1281_panel_power(panel, 0, 1);

		otm1281_panel_on(path);
	} else {
		/* power off */
		if (plat->plat_onoff)
			plat->plat_onoff(0);
...
static void otm1281_reduced_onoff(struct mmp_panel *panel, int status)
{
	if (status) {
		/* power on */
		otm1281_panel_power(panel, 1, 1);
	} else {
		/* power off */
		otm1281_panel_power(panel, 1, 0);
	}
}
...
static struct mmp_panel panel_otm1281 = {
	...
	.set_onoff = otm1281_onoff,
	.reduced_onoff = otm1281_reduced_onoff,
};
