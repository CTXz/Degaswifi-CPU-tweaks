	...
	int size, char *name)
{
	...
	for (i = 0; i < size; i++) {
		...
		if ((temp_packet & SR352_DELAY) == SR352_DELAY) {
			...
			msleep(m_delay*10+10);/*step is 10msec*/
			continue;
		}
...
static int sr352_write(struct i2c_client *c, u16 reg,  u16 value)
{
	...
	if(reg == 0xff)
	{
		msleep(value*10+10);  /* Delay 100ms */
		return 0;
	}
		...
		if(sr352_cam_state == SR352_STATE_PREVIEW)
		{
			...
			{							
				...
				case WIDTH_704:
					...
					if(sensor->cam_mode==CAMERA_MODE){
						sr352_write_regs(c,sr352_Enterpreview_704_576,ARRAY_SIZE(sr352_Enterpreview_704_576),"sr352_Enterpreview_704_576");
				...
				case HD_WIDTH:
					...
					if (old_value == PREVIEW_SIZE_1280_720) {
						sr352_t_whitebalance(c,sensor->wb);
						sr352_t_brightness(c,sensor->ev);
						sr352_t_effect(c,sensor->effect);
					}
					break;
			...
			Cam_Printk(KERN_NOTICE "[DHL] AE... Preview Optimize...!!!! \n");
			msleep(100);
...
static int sr352_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	switch (cmd) { 
		case VIDIOC_DBG_G_CHIP_IDENT:
			...
			return v4l2_chip_ident_i2c_client(client, arg, V4L2_IDENT_SR352, 0);		
			...
			const struct i2c_device_id *did)
{
	...
	if (device_create_file(dev_r, &dev_attr_rear_camtype) < 0)
