...
//#define SENSOR_VENDOR_ID  // For Sensor Vendor ID
...
static int s5k4ecgx_regs_table_init(void)
{
	...
	filp = filp_open("/data/media/0/s5k4ecgx_regs.h", O_RDONLY, 0);
	if (IS_ERR(filp)) {
	...
	*((s5k4ecgx_regs_table + s5k4ecgx_regs_table_size) - 1) = '\0';
	...
	printk(KERN_INFO "*****Compeleted %s %d\n", __func__, __LINE__);
	return 0;
}
...
void s5k4ecgx_regs_table_exit(void)
{
	...
	if (s5k4ecgx_regs_table) {
		...
		printk(KERN_INFO "*****s5k4ecgx_regs_table is already null\n");
...
//			printk(KERN_INFO "addr 0x%04x, value 0x%04x\n", addr, value);

			if (addr == 0xffff) {
				msleep(value);
				printk(KERN_INFO "delay 0x%04x, value 0x%04x\n", addr, value);
			} else {
				if (s5k4ecgx_write(c, addr, value) < 0) {
					printk(KERN_INFO "<=PCAM=> %s fail on sensor_write\n", __func__);
				}
			}
		}
...
static short s5k4ecgx_regs_max_value(char *name)
{
	...
	*(data_buf + 6) = '\0';
	...
	printk(KERN_INFO "s5k4ecgx_regs_max_value done\n");
				...
				switch (mf->width){
					...
					case QVGA_WIDTH:
						...
						Cam_Printk(KERN_ERR"choose qvga(320x240) Preview setting \n");
							...
							Cam_Printk(KERN_ERR"choose qvga(320x240) Camcoder Preview setting \n");
						}
						break;
					...
					case VGA_WIDTH:
						...
						Cam_Printk(KERN_ERR"choose vga(640x480) Preview setting \n");
							...
							Cam_Printk(KERN_ERR"choose vga(640x480) Camcoder Preview setting \n");
						}
...
static int s5k4ecgx_t_saturation(struct i2c_client *client, int value)
{
	...
	switch(value){
		...
		default:
			printk(S5K4ECGX_MOD_NAME "saturation value is not supported!!!\n");
		return -EINVAL;
	}
...
static int s5k4ecgx_t_brightness(struct i2c_client *client, int value)
{
	...
	switch(value){
		...
		default:
			printk(S5K4ECGX_MOD_NAME "brightness value is not supported!!!\n");
		return -EINVAL;
	}
...
static int s5k4ecgx_t_contrast(struct i2c_client *client, int value)
{
	...
	switch(value){
		...
		default:
			printk(S5K4ECGX_MOD_NAME "contrast value is not supported!!!\n");
		return -EINVAL;
	}
...
static int s5k4ecgx_t_effect(struct i2c_client *client, int value)
{
	...
	switch(value){
		...
		default:
			printk(S5K4ECGX_MOD_NAME "effect value is not supported!!!\n");
		return -EINVAL;
	}
...
static int s5k4ecgx_t_photometry(struct i2c_client *client, int value)
{
	...
	switch(value){
		...
		default:
			printk(S5K4ECGX_MOD_NAME "photometry value is not supported!!!\n");
		return -EINVAL;
	}
...
static int s5k4ecgx_t_sharpness(struct i2c_client *client, int value)
{
	...
	switch(value){
		...
		default:
			printk(S5K4ECGX_MOD_NAME "sharpness value is not supported!!!\n");
		return -EINVAL;
	}
...
#ifdef SENSOR_VENDOR_ID
		VENDOR_ID = s5k4ecgx_vendor_id(client);
#endif
		if (!ret) {
