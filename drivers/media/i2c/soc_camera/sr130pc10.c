...
struct device *dev_f;
extern int camera_antibanding_val; /*default : CAM_BANDFILTER_50HZ = 2*/
...
#define COPY_FROM_60HZ_TABLE(TABLE_NAME, ANTI_BANDING_SETTING) \
	...
	COPY_FROM_60HZ_TABLE (sr130pc10_preview_640x480, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_preview_352x288, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_preview_320x240, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_preview_176x144, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_Auto_fps, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_7fps, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_10fps, 60hz);
	COPY_FROM_60HZ_TABLE (sr130pc10_15fps, 60hz);
	printk("%s: copy done!\n", __func__);
	return 0;
}
...
#define IS_SAME_NUM_OF_ROWS(TABLE_NAME) \
	...
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_preview_640x480) ) return (-3);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_preview_352x288) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_preview_320x240) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_preview_176x144) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_Auto_fps) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_7fps) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_10fps) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(sr130pc10_15fps) ) return (-4);
	printk("%s: Success !\n", __func__);
	return 0;
}
...
static int sr130pc10_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (camera_antibanding_val == 3) { /* CAM_BANDFILTER_60HZ  = 3 */
		rc = sr130pc10_check_table_size_for_60hz();
		if(rc != 0) {
...
static int sr130pc10_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	case V4L2_MBUS_FMT_VYUY8_2X8:
		...
		if(sr130pc10_cam_state == SR130PC10_STATE_PREVIEW || sr130pc10_cam_state == SR130PC10_STATE_CAMCORDER){
			switch (mf->width)
			{							
				case QCIF_WIDTH:
...
static int sr130pc10_ESD_check(struct i2c_client *client, struct v4l2_control *ctrl)
{	
...
static int sr130pc10_set_preview_status(int value)
{
	...
	if (value == SR130PC10_STATE_PREVIEW)
...
static int sr130pc10_g_register(struct v4l2_subdev *sd,  struct v4l2_dbg_register * reg)
{
	...
	switch (reg->reg) {
		...
		case V4L2_CID_ESD_CHECK:
		{
			...
			sr130pc10_ESD_check(client, &ESDCtrl);
...
static DEVICE_ATTR(front_camtype, S_IRUGO | S_IXOTH,  front_camera_type, NULL);
			...
			const struct i2c_device_id *did)
{
	...
	// Camera Class already registered at the SR130PC10 in DEGAS model
	dev_f= device_create(camera_class, NULL, 0, "%s", "front");	
	if (device_create_file(dev_f, &dev_attr_front_camtype) < 0)
