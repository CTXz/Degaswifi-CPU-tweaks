...
static inline int ecs_read_bb(struct i2c_client *client, u8 reg, u8 *val)
{
	int ret;
	struct i2c_msg msg[] = {
		{
...
{
	return ecs_read_bb(xic->client, (u8) reg, val);
}
EXPORT_SYMBOL(xic_read_bb);

/* write a register */
static inline int ecs_write_bb(struct i2c_client *client, u8 reg, u8 val)
{
	struct i2c_msg msg;
...
{
	return ecs_write_bb(xic->client, (u8) reg, val);
}
EXPORT_SYMBOL(xic_write_bb);

int xic_write_array_bb(struct x_i2c *xic, const void *table, int count)
{
	struct i2c_client *c = xic->client;
	const struct reg_tab_bb *vals = table;
	int ret, i = 0;

	for (i = 0; i < count; i++) {
		ret = ecs_write_bb(c, vals->reg, vals->val);
		if (unlikely(ret < 0)) {
			dev_err(&c->dev, "I2C returns error code 0x%X\n", ret);
			break;
		}
...
EXPORT_SYMBOL(xic_write_array_bb);
			...
			const u8 *values, int cnt)
{
	...
	memcpy(pdata, &start_addr, 1);
	pdata += 1;
	memcpy(pdata, values, cnt);
	ret = i2c_master_send(c, buffer, 1 + cnt);
	if (ret < 0)
		return ret;
	return 0;
}
EXPORT_SYMBOL(xic_write_burst_bb);
...
int xic_detect_bb(struct x_i2c *xic)
{
	...
	for (i = 0; i < xic->ident_regs; i++) {
		ret = ecs_read_bb(c, xic->ident_addr[i], &val);
		if (unlikely(ret < 0)) {
			dev_err(&c->dev, "I2C returns error code 0x%X\n", ret);
			return -ENXIO;
		}
