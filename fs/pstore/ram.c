...
#ifdef CONFIG_OF
static unsigned long __initdata ramoops_base;
static u32 __initdata ramoops_size;
#endif
...
#ifdef CONFIG_OF
static int __init mmp_ramoops_fdt_find_info(unsigned long node, const char *uname,
		int depth, void *data)
{
	__be32 *prop;
	unsigned long len;

	if (!of_flat_dt_is_compatible(node, "marvell,ramoops-heap"))
		return 0;

	prop = of_get_flat_dt_prop(node, "ramoops-base", &len);
	if (!prop || (len != sizeof(unsigned long))) {
		pr_err("%s: Can't find ramoops-base property\n", __func__);
		return 0;
	}

	ramoops_base = be32_to_cpu(prop[0]);

	prop = of_get_flat_dt_prop(node, "ramoops-size", &len);
	if (!prop || (len != sizeof(unsigned long))) {
		pr_err("%s: Can't find ramoops-size property\n", __func__);
		return 0;
	}

	ramoops_size = be32_to_cpu(prop[0]);

	return 1;
}

void __init mmp_reserve_ramoops(void)
{
	if (of_scan_flat_dt(mmp_ramoops_fdt_find_info, NULL)) {
		BUG_ON(memblock_reserve(ramoops_base, ramoops_size));
		pr_info("Reserved ramoops memory: %dKB at %#.8x\n",
			(unsigned)ramoops_size/0x400,
			(unsigned)ramoops_base);
	} else {
		pr_info("Reserved ramoops memory dt prop fails:: %dKB at %#.8x\n",
			(unsigned)ramoops_size/0x400,
			(unsigned)ramoops_base);
	}
}
