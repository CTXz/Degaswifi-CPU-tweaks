...
#endif
	d2199_aad_write(client, D2199_ACCDET_THRESH4, 0x44);
	/* high power button thres1 */
...
static int d2199_init_setup_register(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, D2199_ALC_CTRL1,
				D2199_ALC_CALIB_MODE_MAN, 0);
	...
	if (d2199_aad->switch_data.state == D2199_HEADSET)
		snd_soc_write(codec, D2199_MICBIAS1_CTRL,
			D2199_MICBIAS_EN | D2199_MICBIAS_LEVEL_2_6V);
	else
		snd_soc_write(codec, D2199_MICBIAS1_CTRL,
...
int d2199_codec_power(struct snd_soc_codec *codec, int on, int clear)
{
	...
	struct i2c_client *client = d2199_codec->aad_i2c_client;
	struct d2199_aad_priv *d2199_aad = i2c_get_clientdata(client);
	struct regulator *aud1_regulator, *aud2_regulator;
	int ret;
	aud1_regulator = regulator_get(NULL, "D2199-LDO_AUD1");
	if (IS_ERR(aud1_regulator)) {
		dlg_info("%s() AUD1 regulator get failed\n", __func__);
		return -1;
	}

	aud2_regulator = regulator_get(NULL, "D2199-LDO_AUD2");
	if (IS_ERR(aud2_regulator)) {
		dlg_info("%s() AUD2 regulator get failed\n", __func__);
		return -1;
	}
	...
	if (on == 1 && d2199_codec->power_mode == 0) {
		...
		ret = regulator_enable(aud1_regulator);
		if (ret)
			dlg_err("%s () AUD1 regulator failed to enable\n", __func__);
		ret = regulator_enable(aud2_regulator);
		if (ret)
			dlg_err("%s () AUD2 regulator failed to enable\n", __func__);
		...
		regulator_disable(aud2_regulator);
		regulator_put(aud2_regulator);


		regulator_disable(aud1_regulator);
		regulator_put(aud1_regulator);
...
static int d2199codec_i2c_read_device(struct d2199_codec_priv *d2199_codec,
				char reg, int bytes, void *dest);
...
void d2199_register_dump(struct snd_soc_codec *codec)
{
	...
	for (i = 0 ; i < 250 ; i++) {
		...
		pr_info("[WS][%s] reg=0x%x val=0x%x\n",  __func__,
						i, val);
	}
}
 ...
 * Control list
 */
static unsigned int d2199_codec_read(struct snd_soc_codec *codec,
					unsigned int reg);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	d2199_codec->aif2_enable = !!(val & 0x80) ? 1 : 0;
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	d2199_codec->aif1_master = !!(val & 0x80) ? 1 : 0;
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	d2199_codec->bt_enable = !!(val & 0x01) ? 1 : 0;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	d2199_codec->narrow_band = ((val & 0x80) == 0x1) ? 1 : 0;
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	d2199_codec->fm_radio = !!(val & 0x80) ? 1 : 0;

	return snd_soc_put_volsw(kcontrol, ucontrol);

}

extern void d2199_aad_button_fast_rate(struct d2199_aad_priv *d2199_aad,
						int enable);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (((val & 0x80) == 0) &&
			d2199_aad->switch_data.state == D2199_HEADSET) {
		d2199_aad_button_fast_rate(d2199_aad, 1);
		mutex_unlock(&d2199_codec->power_mutex);
		return 0;
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	d2199_codec->hp_enable = ((val & 0x80) == 0x80) ? 1 : 0;
...
static const struct snd_kcontrol_new d2199_snd_controls[] = {
	...
	SOC_SINGLE("D2199_DROUTING_DAI_1_L", D2199_DROUTING_DAI_1_L,
			0, 0xff, 0),
	SOC_SINGLE("D2199_DROUTING_DAI_1_R", D2199_DROUTING_DAI_1_R,
			0, 0xff, 0),
	SOC_SINGLE("D2199_DROUTING_DAI_2_L", D2199_DROUTING_DAI_2_L,
			0, 0xff, 0),
	SOC_SINGLE("D2199_DROUTING_DAI_2_R", D2199_DROUTING_DAI_2_R,
			0, 0xff, 0),
	SOC_SINGLE("D2199_DROUTING_DAC_L", D2199_DROUTING_DAC_L, 0, 0xff,
		   0),
	SOC_SINGLE("D2199_DROUTING_DAC_R", D2199_DROUTING_DAC_R, 0, 0xff,
		   0),
#if defined(CONFIG_MACH_KAVERILTE) || defined(CONFIG_MACH_YANGZILTE)
	SOC_SINGLE("D2199_DMIX_DAC_R_DAI_2_L_GAIN",
		D2199_DMIX_DAC_R_DAI_2_L_GAIN, 0, 0xff, 0),
	SOC_SINGLE("D2199_DMIX_DAC_R_DAI_2_R_GAIN",
		D2199_DMIX_DAC_R_DAI_2_R_GAIN, 0, 0xff, 0),
#endif
	...
	/*SOC_SINGLE("D2199_DAI_2_CLK_MODE", D2199_DAI_2_CLK_MODE,
	 * 0, 0xff, 0),*/
	...
	SOC_SINGLE("D2199_CP_VOL_THRESHOLD1", D2199_CP_VOL_THRESHOLD1,
			0, 0xff, 0),
	...
	SOC_SINGLE("D2199_MIXOUT_SP_CTRL", D2199_MIXOUT_SP_CTRL,
		   0, 0xff, 0),
	SOC_SINGLE("D2199_MIXOUT_SP_SELECT", D2199_MIXOUT_SP_SELECT,
			0, 0xff, 0),
			   ...
			   struct snd_soc_dai *dai)
{
	...
	/*snd_soc_update_bits(codec, D2199_DAI_1_CTRL,
		 * D2199_DAI_WORD_LENGTH_MASK, aif_ctrl);
...
static int d2199_probe(struct snd_soc_codec *codec)
{
	...
	if (d2199_codec->clk) {
		clk_prepare_enable(d2199_codec->clk);
...
static int d2199_codec_write(struct snd_soc_codec *codec,
...
static int d2199codec_i2c_write_device(struct d2199_codec_priv *d2199_codec,
		char reg, int bytes, u8 *src)
{
	int ret;
 ...
 *
 */
static long d2199_codec_ioctl(struct file *file,
				unsigned int cmd, unsigned long arg)
{
		...
		case D2199_IOCTL_READ_REG:
			...
			d2199codec_i2c_read_device(d2199_codec, (u8)(reg.reg),
					1, (void *)(&reg_val));
		...
		case D2199_IOCTL_WRITE_REG:
			...
			d2199codec_i2c_write_device(d2199_codec,
					(char)(reg.reg), 1,  (u8 *)(&reg.val));
...
{
	pr_info("Usage:\n");
	pr_info("Read a register: echo 0x0800 > /proc/codec0\n");
	pr_info("Read multiple regs: echo 0x0800 -c 10 > /proc/codec0\n");
	pr_info("Write multiple regs: echo 0x0800 0xFF 0xFF > /proc/codec0\n");
	pr_info("Write single reg: echo 0x0800 0xFF > /proc/codec0\n");
 ...
 *
 */
static ssize_t d2199_codec_ioctl_write(struct file *file,
		const char __user *buffer, size_t len, loff_t *offset)
{
	...
	if (dbg.read_write == CODECDBG_READ_REG) {
		...
		for (i = 0; i < dbg.len; i++, dbg.addr++) {
			ret = d2199codec_i2c_read_device(d2199_codec,
					dbg.addr, 1, &dbg.val[i]);
		...
		for (i = 0; i < dbg.len; i++, dbg.addr++) {
			...
			ret = d2199codec_i2c_write_device(d2199_codec,
					dbg.addr, 1, &dbg.val[i]);

			if (ret < 0) {
				dlg_err("%s: codec reg write"
					"failed ret=%d\n", __func__, ret);
				return -EFAULT;
			}
		}
...
void d2199_codec_debug_proc_init(struct d2199_codec_priv *d2199_codec)
{
	...
	entry = proc_create_data("codec0", S_IRWXUGO, NULL,
			&d2199_codec_ops, d2199_codec);
		dlg_crit("\nD2199.c: proc_create_data() = %p\n", entry);
}
...
static int d2199_i2c_probe(struct i2c_client *client,
...
static int d2199_i2c_suspend(struct i2c_client *client,
				pm_message_t mesg)
{
...
static int d2199_i2c_resume(struct i2c_client *client)
{
	struct d2199_codec_priv *d2199_codec = i2c_get_clientdata(client);
...
static int d2199_i2c_remove(struct i2c_client *client)
{
#ifdef CONFIG_SND_SOC_D2199_AAD
struct d2199_codec_priv *d2199_codec = i2c_get_clientdata(client);
#endif
