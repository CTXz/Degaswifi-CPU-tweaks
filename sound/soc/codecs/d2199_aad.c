...
static void d2199_aad_g_monitor_timer_work(struct work_struct *work)
{
	...
	if (d2199_aad->g_status == 0  &&
		...
		/*incomplet insertion*/
		d2199_aad->g_count++;
		if (d2199_aad->g_count == 5) {
			/* there is no jack detection during 1 sec*/
			schedule_delayed_work(&d2199_aad->jack_monitor_work,
			msecs_to_jiffies(5));
		} else {
...
static void d2199_aad_jack_monitor_timer_work(struct work_struct *work)
{
	...
	dlg_info("%s\n", __func__);
#endif
	if (d2199_aad->d2199_codec == NULL ||
			d2199_aad->d2199_codec->codec_init == 0) {
		schedule_delayed_work(&d2199_aad->jack_monitor_work,
	...
	if (d2199_aad->switch_data.state != D2199_NO_JACK) {
		...
		if ((jack_mode & D2199_ACCDET_JACK_MODE_JACK) == 0) {
			...
			if (d2199_aad->switch_data.state != state) {
				...
				switch_set_state(&d2199_aad->switch_data.sdev,
						state);
			}
	...
	if (jack_mode & D2199_ACCDET_JACK_MODE_JACK) {
		...
		if (d2199_aad->switch_data.state != D2199_NO_JACK) {
			...
			if (d2199_aad->switch_data.state == D2199_HEADSET
				&& (jack_mode & 0x80) == 0) {
				d2199_aad_write(client,
					D2199_ACCDET_CONFIG, 0x00);
				usleep_range(1000, 1000);
				d2199_aad_write(client, D2199_ACCDET_CONFIG, 0x88);
		...
		while (1) {
			...
			btn_status2 = d2199_aad_read(client,
					D2199_ACCDET_STATUS);
#ifdef D2199_AAD_DEBUG
			dlg_info("%s 2Current Heaset set ADC= %d\n",
					__func__, btn_status2);
#endif
			if (btn_status2 >= btn_status1 || btn_status1 == 0 ||
							btn_status2 == 0) {
...
static void d2199_aad_button_monitor_timer_work(struct work_struct *work)
{
	...
	if (d2199_aad->button.status == D2199_BUTTON_RELEASE) {
		d2199_aad->button.b_status =
			snd_soc_read(d2199_aad->d2199_codec->codec,
					D2199_MICBIAS1_CTRL);
	}
...
static ssize_t switch_sec_get_key_state(struct device *dev,
...
static ssize_t switch_sec_get_state(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
/* sysfs for headset state - ffkaka */
static ssize_t switch_sec_get_adc(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 adc;
	...
	d2199_aad_ex->button.b_status =
			snd_soc_read(d2199_aad_ex->d2199_codec->codec,
					D2199_MICBIAS1_CTRL);
...
static int d2199_aad_i2c_probe(struct i2c_client *client,
					 const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&d2199_aad->jack_monitor_work,
			d2199_aad_jack_monitor_timer_work);
	INIT_DELAYED_WORK(&d2199_aad->button_monitor_work,
			d2199_aad_button_monitor_timer_work);
	INIT_DELAYED_WORK(&d2199_aad->g_monitor_work,
			d2199_aad_g_monitor_timer_work);
	...
	d2199_aad->g_det_irq = d2199_aad->d2199_codec->
		d2199_pmic->pdata->jack_irq;
	d2199_aad->gpio =
		pxa_irq_to_gpio(d2199_aad->g_det_irq);

	dlg_info("[%s] : qpio_to_irq= %d\n",
		__func__, d2199_aad->g_det_irq);
	ret = request_threaded_irq(d2199_aad->g_det_irq, NULL,
			d2199_g_det_handler, IRQF_TRIGGER_FALLING |
			IRQF_TRIGGER_RISING | IRQF_DISABLED,
			"GPIO detect", d2199_aad);
	...
	ret = request_threaded_irq(irq, NULL, d2199_jack_handler,
				...
				IRQF_DISABLED, "Jack detect", d2199_aad);
	...
	sec_earjack_dev = device_create(sec_audio_earjack_class, NULL,
			0, NULL, "earjack");
...
static int d2199_aad_i2c_remove(struct i2c_client *client)
{
	struct d2199_aad_priv *d2199_aad = i2c_get_clientdata(client);
