...
struct tdm_dai_private {
	...
	int tdm_out2_tx_num;
	unsigned int tdm_codec_mic1_rx;
	int tdm_codec_mic1_rx_num;
	unsigned int tdm_codec_mic2_rx;
				...
				unsigned int reg)
{
	...
	struct pm860_chip *chip = pm860_priv->chip;
	...
	if (reg >= PMIC_INDEX) {
		...
		ret = pm822_extern_read(PM822_POWER_PAGE,
				reg);
		if (ret < 0)
			return -EINVAL;
		else
	...
	unsigned int value)
{
	...
	if (reg >= PMIC_INDEX) {
		...
		ret = pm822_extern_write(PM822_POWER_PAGE,
			reg, value);
		if (ret < 0)
			dev_err(chip->dev,
				...
				unsigned int rx_num, unsigned int *rx_slot)
{
	...
	tdm_dai_priv->tdm_codec_mic1_rx = rx_slot[0];
	tdm_dai_priv->tdm_codec_mic1_rx_num = 1;
				...
				unsigned int rx_num, unsigned int *rx_slot)
{
	...
	tdm_dai_priv->tdm_codec_mic2_rx = rx_slot[0];
	tdm_dai_priv->tdm_codec_mic2_rx_num = 1;
...
static void pm860_reg_init(struct snd_soc_codec *codec)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(codec_pdata); i++)
		snd_soc_write(codec, codec_pdata[i].addr, codec_pdata[i].val);
}
...
{
	struct pm860_chip *chip = dev_get_drvdata(pdev->dev.parent);
