...
#define MXT_DEV_NAME				"mxt224s"
#define MXT_DEFAULT_FIRMWARE_NAME	"mxts.fw"
#endif

#define FW_UPDATE_ENABLE		0

#define MXT_FIRMWARE_INKERNEL_PATH	"atmel"
...
struct mxt_platform_data {
	...
	u32 mxts_vdd_regulator_volt;
	u32 mxts_1v8_regulator_volt;
	...
	const u8 **config;
	bool (*read_chg)(struct i2c_client *);
	int (*power_setup) (struct i2c_client *, bool);
	int (*power_onoff) (struct i2c_client *, bool);
...
struct mxt_data {
	...
	int tcount[10];
	u16 touchbx[MAX_USING_FINGER_NUM];
	u16 touchby[MAX_USING_FINGER_NUM];
	u16 touchbx_backup[MAX_USING_FINGER_NUM];
	u16 touchby_backup[MAX_USING_FINGER_NUM];
#endif
#if TSP_BOOSTER
...
#define MXT_DT_FLAG_GPIO_INT			(0x1<<0)
#define MXT_DT_FLAG_REGISTER_CB			(0x1<<1)
#define MXT_DT_FLAG_GPIO_REGULATOR		(0x1<<2)
#define MXT_DT_FLAG_COMMON_REGULATOR		(0x1<<3)
#endif
#endif
