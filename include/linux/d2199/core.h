...
#ifndef __D2199_CORE_H_
#define __D2199_CORE_H_
...
struct d2199 {
	...
	int (*read_dev)(struct d2199 * const d2199, char reg, int size,
			void *dest);
	int (*write_dev)(struct d2199 * const d2199, char reg, int size,
			u8 *src);
 ...
 * d2199 device IO
 */
int	d2199_clear_bits(struct d2199 * const d2199, u8 const reg,
			 u8 const mask);
...
/* DLG IOCTL interface */
extern int d2199_ioctl_regulator(struct d2199 *d2199, unsigned int cmd,
				 unsigned long arg);
