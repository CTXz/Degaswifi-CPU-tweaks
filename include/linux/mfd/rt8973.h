...
enum {
	...
	MUIC_RT8973_CABLE_TYPE_USB,	/* adc 0x1f (no id) */
	MUIC_RT8973_CABLE_TYPE_OTG,	/* adc 0x00, regDev1&0x01 */

	/* TA Group */
	MUIC_RT8973_CABLE_TYPE_REGULAR_TA,	/* adc 0x1f
						 * (no id, D+ short to D-) */
	MUIC_RT8973_CABLE_TYPE_ATT_TA,		/* adc 0x1f
						 * (no id, only VBUS)AT&T */
	...
	MUIC_RT8973_CABLE_TYPE_CDP,		/* USB CDP,
						 * usually treated as SDP */
...
struct rt8973_platform_data {
	...
	struct pinctrl *p;
	struct pinctrl_state *default_pins_state;
	struct pinctrl_state *remove_pins_state;
};
